...
export function IndustriesOperations({ language }: { language: Language }) {
  ...
  return (
    <section id="industrias" className="section-bg section-contained border-t border-white/10 py-24">
      <SectionBackground image={operationsImg} position="center" opacity={0.2} overlay="dark" />
      ...
      <Reveal className="relative z-10 mx-auto grid max-w-7xl gap-12 px-5 md:px-8 lg:grid-cols-[.92fr_1.08fr] lg:items-center">
        <ImagePanel
          ...
          title={
            isEnglish ? "Operational systems, not generic apps" : "Sistemas de operación, no aplicaciones genéricas"
          }
          text={
            isEnglish
