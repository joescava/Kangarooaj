...
import {
  ...
  CONTACT_PHONE_LABEL,
  WHATSAPP_URL,
} from "@/features/landing/constants/contact";
import { services } from "@/features/landing/constants/services";
...
export function Contact({
  ...
  const { form, submitted, getFieldError, markSubmitted, resetSubmitted } =
      ...
      <Reveal className="relative z-10 fluid-container max-w-6xl">
        <div className="grid min-w-0 gap-8 overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.045] p-4 shadow-2xl backdrop-blur-xl sm:p-5 md:p-8 lg:grid-cols-[.9fr_1.1fr]">
          <div className="relative min-w-0 overflow-hidden rounded-3xl border border-white/10 bg-slate-950/45 p-5 sm:p-7 md:p-8">
            ...
            <p className="localized-copy mt-6 leading-8 text-slate-300">
              {t.contactText}
            </p>
          ...
          >
            ...
            <label className="flex min-w-0 items-start gap-3 rounded-2xl border border-white/10 bg-slate-950/40 p-4 text-sm leading-6 text-slate-300">
              <input
                ...
                required
                aria-invalid={Boolean(getFieldError("acceptedPrivacy"))}
                aria-describedby={
                  getFieldError("acceptedPrivacy")
                    ? "acceptedPrivacy-error"
                    : undefined
                }
                onFocus={trackStart}
              ...
              <span>
                {t.privacyConsent}{" "}
                <a
                  href={`${import.meta.env.BASE_URL}${region === "CO" ? "data-treatment-policy.html" : "privacy-policy.html"}`}
                  target="_blank"
